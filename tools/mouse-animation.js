...
function CanvasDraw(elem) {
  ...
  this.drawHeart = function(x, y, r, d, color) {
    canvas.beginPath()
    canvas.moveTo(x, y)
    canvas.bezierCurveTo(x - d / 6, y - r / 2, x - d, y - r / 2, x - d, y)
    canvas.bezierCurveTo(x - d, y + r / 2, x - d / 6, y + r - r / 4, x, y + r)
    canvas.bezierCurveTo(x + d / 6, y + r - r / 4, x + d, y + r / 2, x + d, y)
    canvas.bezierCurveTo(x + d, y - r / 2, x + d / 6, y - r / 2, x, y)
...
export default function mouseMoveAnimation(panelElem) {
  ...
  var preTime = 0
  var timeInterval = 40
  panelElem.onmousemove = function(e) {
    ...
    if (nowTime - preTime >= timeInterval) {
      ...
      var colorStr = 'rgba(' + rn(30, 200) + ',' + rn(30, 200) + ',' + rn(30, 200) + ','
      animate(
        0,
        Math.random() * 30 + 10,
        2000,
        { math: 'Quint', type: 'easeOut' },
        function(value, time) {
          const color = colorStr + (0.9 - (time / 2000) * 1) + ')'
